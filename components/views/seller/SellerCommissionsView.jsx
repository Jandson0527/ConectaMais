    ...
    <section className="view-panel active" id="view-seller-commissions">
      ...
      <div style={{
        ...
        <div style={{
          ...
          <strong style={{ fontSize: '1.25rem', color: commData.pendingCommission > 0 ? 'var(--accent-gold)' : 'var(--success)', fontWeight: 900 }}>
      ...
      <div className="kpi-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <div className="kpi-card" style={{ '--accent-color': 'var(--purple)' }}>
          <div className="kpi-icon-box" style={{ color: 'var(--purple)' }}>
      ...
      <div style={{
        background: 'var(--bg-card)',
        border: '1.5px solid rgba(2, 132, 166, 0.35)',
        borderRadius: '16px',
        padding: '1.5rem',
        marginBottom: '1.5rem',
        boxShadow: 'var(--shadow-sm)'
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
          <div style={{ width: '36px', height: '36px', borderRadius: '8px', background: 'var(--primary-bright)', color: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <div>
              <h3 style={{ margin: 0, fontSize: '1.1rem', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <ArrowDownCircle style={{ width: '18px', height: '18px', color: 'var(--success)' }} />
              ...
              <tbody>
                {commData.payoutsHistory.map(p => (
                  <tr key={p.id} style={{ borderBottom: '1px solid var(--border-subtle)', fontSize: '0.85rem' }}>
                    <td style={{ padding: '10px 14px', color: 'var(--text-primary)', fontWeight: 700 }}>
                      {p.date ? (p.date.includes('-') ? `${p.date.split('-')[2]}/${p.date.split('-')[1]}/${p.date.split('-')[0]}` : p.date) : '-'}
                    </td>
                    <td style={{ padding: '10px 14px', color: 'var(--success)', fontWeight: 800 }}>
                    ...
                    <tr key={item.id || idx} style={{ borderBottom: '1px solid var(--border-subtle)', fontSize: '0.85rem' }} className="table-row-hover">
                      <td style={{ padding: '12px 14px' }}>
                        <span style={{
                          ...
                          color: isRecurring ? 'var(--primary-bright)' : 'var(--accent-gold)',
                      ...
                      <td style={{ padding: '12px 14px', color: 'var(--primary-bright)', fontWeight: 700 }}>
                        10%
                      </td>
