        ...
        {users.map(user => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ position: 'relative' }}>
                <img
                  src={user.avatar}
                  alt={user.name}
                  style={{ width: '52px', height: '52px', borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--primary-bright)', filter: user.active === false ? 'grayscale(1)' : 'none' }}
                />
                {user.active === false && (
                  <span style={{ position: 'absolute', bottom: -5, right: -5, background: 'var(--danger)', color: '#fff', fontSize: '0.6rem', padding: '2px 4px', borderRadius: '4px', fontWeight: 700 }}>INATIVO</span>
                )}
              </div>
              <div>
                <h3 style={{ margin: 0, fontSize: '1.1rem', color: 'var(--text-primary)', textDecoration: user.active === false ? 'line-through' : 'none' }}>{user.name}</h3>
            ...
            <div style={{
              ...
              <strong style={{ color: 'var(--text-primary)', display: 'block', marginBottom: '4px' }}>Permissões ({user.permissions?.length || 0}):</strong>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                {(user.permissions || []).map(p => (
                  <span key={p} style={{ background: 'rgba(0, 210, 255, 0.1)', padding: '2px 6px', borderRadius: '4px', color: 'var(--primary-bright)', fontSize: '0.7rem' }}>
                    {p}
                  </span>
                ))}
              </div>
